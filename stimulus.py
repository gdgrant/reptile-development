...
class Stimulus:
    ...
    def generate_image_batch(self, task_num, test=False):
        ...
        q = np.random.randint(0,len(ind),self.batch_size)

        # Pick out batch data and labels
        batch_data   = np.zeros((self.batch_size, 32,32,3), dtype = np.float32)
        batch_labels = np.zeros((self.batch_size, par['layer_dims'][-1]), dtype = np.float32)

        # Build multihead mask
        if par['multihead']:
            mask = np.zeros((self.batch_size, par['layer_dims'][-1]), dtype = np.float32)
            mask[:, task_num*self.labels_per_task:(task_num+1)*self.labels_per_task] = 1
        else:
            mask = np.ones((self.batch_size, par['layer_dims'][-1]), dtype = np.float32)
    ...
    def generate_mnist_batch(self, task_num, test=False):
        ...
        q = np.random.randint(0, ind_num, self.batch_size)

        # Pick out batch data and labels
        batch_data   = np.zeros((self.batch_size, 28**2), dtype=np.float32)
        batch_labels = np.zeros((self.batch_size, self.num_outputs), dtype=np.float32)
        for i in range(self.batch_size):
            if test:
                # Select images and labels from the testing set if required
    ...
    def make_batch(self, batch_size, task_num, test=False):
        ...
        if par['task'] == 'mnist':
            ...
            mask = np.ones((self.batch_size, 10), dtype=np.float32)
